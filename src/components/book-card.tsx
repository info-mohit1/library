    ...
    <Card className="flex flex-col h-full overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1 bg-card group border-border/50">
      <div className="relative aspect-[2/3] overflow-hidden bg-muted/30">
        ...
        <div className="absolute top-2 right-2">
          <Badge
            variant="secondary"
            className={`text-xs font-medium backdrop-blur-sm ${CATEGORY_COLORS[book.category] ?? "bg-background/80"}`}
          >
            {book.category}
          </Badge>
        </div>
      ...
      <CardContent className="flex-1 p-4 flex flex-col gap-1">
        ...
        <div className="mt-auto pt-4 flex items-center justify-between text-xs">
          <span
            className={`${book.available_quantity > 0 ? "text-secondary" : "text-destructive"} font-medium`}
          >
      ...
      <CardFooter className="p-4 pt-0">
        <Link href={`/book/${book.id}`} className="w-full">
          <Button
            variant="outline"
            className="w-full border-border/60 hover:border-primary hover:text-primary transition-all"
          >
            View Details
          </Button>
        </Link>
